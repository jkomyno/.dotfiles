#!/usr/bin/env python3
"""Apply configured frontmatter overrides to a vendored skill SKILL.md."""
...
def main(argv: list[str]) -> int:
    ...
    flags = set(argv[4:])
    allowed = {"--quiet", "--strip"}
    if (flags - allowed) or len(flags) != len(argv[4:]):
        usage()
        return 2

    changed = (
        strip_overrides(skill_name, skill_md, patches_dir)
        if "--strip" in flags
        else apply_overrides(skill_name, skill_md, patches_dir)
    )
    if changed and "--quiet" not in flags:
        print("OVERRIDE: SKILL.md frontmatter")
    return 0
