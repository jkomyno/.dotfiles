# Orchestrated mode: leaves as fresh delegated agents
...
not time, is the scarce resource, and a fresh delegated agent per leaf resets it.
...
1. **Plan.** Write PLAN.md (contract, tree, gates file per leaf and branch)
   ...
   fit in one head.
2. **Dispatch one leaf.** Delegate to a fresh agent whose entire brief is:
...
run concurrently if the harness supports parallel delegated agents. Parallelism
...
Where the harness allows choosing a model or reasoning effort per delegated agent,
...
Below roughly half an hour of real work, delegated-agent overhead (context
