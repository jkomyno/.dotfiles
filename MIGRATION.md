...
- Stage 4 now has a staged setup runner. `mise run setup:staged -- --plan`
  ...
  hook templates without running installers. The runner supports filtered
  slices with `--from`, `--until`, `--only`, and `--skip` so isolated test runs
  can resume at a specific step.
